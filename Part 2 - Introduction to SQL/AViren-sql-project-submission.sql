-- Programming for Data Science Nano Degree August 14, 2018 Cohort
-- Project 1: Investigate a Database Submission (postgres: 'dvdrental')
-- Andy Viren
-- *********************************************************************

-- Slide #1 What are our total sales per hour?
WITH 
	qryReturnsByHour 
	AS(
	SELECT 
...
SELECT 
	...
	avg(SumByDay)::int as avgPerHour, date_part('hour',returnHour),
	storeNum
FROM qryRunningSum
GROUP BY returnHourUnique, storeNum, date_part('hour',returnHour)
ORDER BY storeNum, date_part('hour',returnHour)

-- Slide #2 Which special feature is associated with the most film sales?
...
-- Slide #3 How full does the stock room get each day?
WITH 
	qryReturnsByHour 
	AS(
	SELECT 
...
SELECT 
	...
	storeNum,
	extract(hour from returnHour)
FROM qryRunningSum
GROUP BY returnHourUnique, storeNum, extract(hour from returnHour)
ORDER BY storeNum, extract(hour from returnHour)


-- Slide #4 Which countries have the highest rentals per capita?
-- Use data from https://www.photius.com/rankings/population/population_2006_0.html
